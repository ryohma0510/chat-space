...
describe Message do
  describe '#create' do
    ...
    it 'is valid with a content, user_id, group_id && pass the foreign_key constraint' do
      message = create(:message, user_id: user.id, group_id: group.id) #外部キー認証が通るかも試したいのでcreateを使った
      expect(message).to be_valid
    end

    it 'is invalid when content is nil' do
      message = build_stubbed(:message, content: '')
      message.valid?
      expect(message.errors[:content]).to include('を入力してください。')
    end

    it 'is invalid when user_id is nil' do
      message = build_stubbed(:message, user_id: '')
      message.valid?
      expect(message.errors[:user_id]).to include('を入力してください。')
    end

    it 'is invalid when group_id is nil' do
      message = build_stubbed(:message, group_id: '')
      message.valid?
